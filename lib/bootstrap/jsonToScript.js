...
const createScript = (item, editorInterface) => `// ts-check
export const description = "Create content model for ${item.props.name}";

  /**
   * @param {import("contentful-migration").default} migration
   */
  export const up = (migration) => {${createContentType(item, editorInterface)}};

  /**
   * @param {import("contentful-migration").default} migration
   */
  export const down = migration => migration.deleteContentType("${item.id}");
`
